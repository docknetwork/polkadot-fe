...
import { useAccounts, useApi, useCall, useCallMulti, useInflation, calcDockInflation } from '@polkadot/react-hooks';
...
export default function useSortedTargets (favorites: string[], withLedger: boolean): SortedTargets {
  ...
  // DOCK: Total yearly emission
  const yearly = useCall<BN>(totalIssuance && baseInfo?.totalStaked && api.rpc.staking_rewards.yearlyEmission, [baseInfo?.totalStaked?.toString(), totalIssuance?.toString()]);

  const inflation = useInflation(baseInfo?.totalStaked, yearly);

  const partial = useMemo(
    () => inflation && inflation.stakedReturn
      ? addReturns(inflation, baseInfo)
      : baseInfo,
    [baseInfo, inflation, yearly]
  );
